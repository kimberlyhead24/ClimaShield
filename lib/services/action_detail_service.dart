...
class ActionDetailService {
  ...
  Future<Map<String, dynamic>> getResolvedAction(String actionId) async {
    ...
    final double gridFactor = (solar['grid_emissions_factor'] as num).toDouble();
    final int annualKwh = (solar['annual_kwh'] as num).toInt();
    final int co2ePerYear = (solar['co2e_reduction_per_year_kg'] as num).toInt();
    ...
    final replacements = {
      ...
      '{{step4_co2e}}': ((peakSunHours * 7000 * 0.365) * gridFactor).round().toString(),
      ...
      '{{panel_count_example}}': ((30 / peakSunHours * 1000) / 400).ceil().toString(),
    };
  ...
  double _latitudeForState(String code) {
    const latitudes = {
      ...
      'CO': 39.5, 'CT': 41.6, 'DE': 39.0, 'FL': 27.8, 'GA': 32.2,
      'HI': 20.8, 'ID': 44.5, 'IL': 40.6, 'IN': 40.3, 'IA': 42.0,
      'KS': 38.5, 'KY': 37.8, 'LA': 31.2, 'ME': 45.3, 'MD': 39.0,
      'MA': 42.3, 'MI': 44.3, 'MN': 46.4, 'MS': 32.7, 'MO': 38.4,
      'MT': 46.9, 'NE': 41.5, 'NV': 39.5, 'NH': 43.7, 'NJ': 40.1,
      'NM': 34.5, 'NY': 42.9, 'NC': 35.8, 'ND': 47.5, 'OH': 40.4,
      'OK': 35.6, 'OR': 44.6, 'PA': 41.2, 'RI': 41.7, 'SC': 33.8,
      'SD': 44.4, 'TN': 35.9, 'TX': 31.5, 'UT': 39.3, 'VT': 44.0,
      'VA': 37.8, 'WA': 47.4, 'WV': 38.6, 'WI': 44.5, 'WY': 43.0,
      'DC': 38.9,
    };
